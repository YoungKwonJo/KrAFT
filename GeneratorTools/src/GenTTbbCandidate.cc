...
void GenTTbbCandidate::building( const std::vector<reco::GenJet>* genJets, const reco::GenParticleCollection* genParticles  ){
...
//////
  ...
  for (std::vector<reco::GenJet>::const_iterator genJet=genJets->begin();genJet!=genJets->end();++genJet, ++idx){
    ...
    bool isPAT = true;//genJet->sourcePtr()->isAvailable(); 
...
std::vector<const reco::Candidate *> GenTTbbCandidate::getAncestors(const reco::Candidate &c)
{
...
bool GenTTbbCandidate::hasBottom(const reco::Candidate &c)
{
  int code1;
  int code2;
...
bool GenTTbbCandidate::hasCharm(const reco::Candidate &c) 
{
  int code1;
  int code2;
...
bool GenTTbbCandidate::decayFromBHadron(const Candidate & c)
{
   bool isFromB = false;
...
bool GenTTbbCandidate::decayFromCHadron(const Candidate & c)
{
  bool isFromC = false;
  vector<const Candidate *> allParents = getAncestors( c );
  for( vector<const Candidate *>::const_iterator aParent  = allParents.begin();
...
const Candidate* GenTTbbCandidate::lastBHadron(const Candidate & c)
{
   const Candidate * out;
...
const Candidate* GenTTbbCandidate::lastCHadron(const Candidate & c)
{
   const Candidate * out;
...
bool GenTTbbCandidate::isLastbottom( const reco::GenParticle& p ){
   bool out = true;
...
bool GenTTbbCandidate::isLastcharm( const reco::GenParticle& p ){
   bool out = true;
...
bool GenTTbbCandidate::isLastParton( const reco::GenParticle& p){
...
const reco::Candidate* GenTTbbCandidate::getLast( const reco::Candidate& p ){
...
bool GenTTbbCandidate::isFromtop( const reco::GenParticle& p){
  bool out = false;
...
double GenTTbbCandidate::deltaR( const Candidate& pasObj, const Candidate& proObj ) {
