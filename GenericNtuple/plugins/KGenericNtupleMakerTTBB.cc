...
class KGenericNtupleMakerTTBB : public edm::EDAnalyzer
{
public:
  KGenericNtupleMakerTTBB(const edm::ParameterSet& pset);
  ~KGenericNtupleMakerTTBB();
...
private:
  ...
  // Output tree
  TTree* tree_;
  GenericEventTTBB* fevent_;

};

KGenericNtupleMakerTTBB::KGenericNtupleMakerTTBB(const edm::ParameterSet& pset)
{
  ...
  fevent_ = new GenericEventTTBB(isMC_);
  fevent_->book(tree_);
}

KGenericNtupleMakerTTBB::~KGenericNtupleMakerTTBB()
{
}

void KGenericNtupleMakerTTBB::endLuminosityBlock(const edm::LuminosityBlock& lumi, const edm::EventSetup& eventSetup)
{
  for ( int i=0, n=eventCounterLabels_.size(); i<n; ++i )
  {
...
void KGenericNtupleMakerTTBB::analyze(const edm::Event& event, const edm::EventSetup& eventSetup)
{
  using namespace std;
...
//  fevent_->nVertex_ = vertexHandle->size();
  ...
  if ( isMC_ )
  {
    ...
    for ( int i=0, n=genHandle->size(); i<n; ++i )
    {
      ...
      { 
          for(int j=0, m=p.numberOfMothers();j<m;++j) 
            if(p.mother(j)->pdgId()==21 || p.mother(j)->pdgId()==9 ) gluon2t_N++; 
      }
      if ( p.status() != 3 ) continue;
      if ( p.pt() == 0 ) continue;
      genParticlesToStore.push_back(&p);
    }
    fevent_->gluon2t_N_ = gluon2t_N;
    ...
    for ( int i=0, n=genJetHandle->size(); i<n; ++i )
    {
      ...
      int isdecayFromBHadron_ = 0;
      int isdecayFromCHadron_ = 0;

      std::vector<const reco::GenParticle*> particles = p.getGenConstituents();
     
      // Loop over the constituent particles
      std::vector<const reco::GenParticle*>::const_iterator genit = particles.begin();
      std::vector<const reco::GenParticle*>::const_iterator genend = particles.end();
      for ( ; genit != genend; ++genit ) 
      {
         const reco::GenParticle & genitref = **genit;
         if ( JetMCTagUtils::decayFromBHadron( genitref ) ) isdecayFromBHadron_ = 1;
         if ( JetMCTagUtils::decayFromCHadron( genitref ) ) isdecayFromCHadron_ = 1;
      }
      fevent_->genJets_decayFromBHadron_->push_back(isdecayFromBHadron_);
      fevent_->genJets_decayFromCHadron_->push_back(isdecayFromCHadron_);

    }

    //ttbar gen variables
    vallot::GenTTbbCandidate ttbarGenLevel;
    if( genHandle.isValid() && genJetHandle.isValid() )
    { 
      const reco::GenParticleCollection* myGenParticles = 0;
      myGenParticles = &(*genHandle);
      const std::vector<reco::GenJet>* myGenJets = 0;
      myGenJets = &(*genJetHandle);
    
      ttbarGenLevel.building(myGenJets, myGenParticles);
      fevent_->genttbarM_     = ttbarGenLevel.mass();
      fevent_->nGenJet20_     = ttbarGenLevel.NJets20();
      fevent_->nGenbJet20_    = ttbarGenLevel.NbJets20(0);
      fevent_->nGenaddbJet20_ = ttbarGenLevel.NaddbJets20(0);
      fevent_->nGencJet20_    = ttbarGenLevel.NcJets20(0);
    
      fevent_->nGenJet40_     = ttbarGenLevel.NJets40();
      fevent_->nGenbJet40_    = ttbarGenLevel.NbJets40(0);
      fevent_->nGenaddbJet40_ = ttbarGenLevel.NaddbJets40(0);
      fevent_->nGencJet40_    = ttbarGenLevel.NcJets40(0);
    
      fevent_->genLep1_pt_ = ttbarGenLevel.lepton1().pt();
...
int KGenericNtupleMakerTTBB::findMother(const reco::GenParticle* p, std::vector<const reco::GenParticle*>& genParticles)
{
  if ( !p ) return -1;
...
DEFINE_FWK_MODULE(KGenericNtupleMakerTTBB);
